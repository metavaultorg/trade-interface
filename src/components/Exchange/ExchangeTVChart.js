...
const getSeriesOptions = (precision,minMove) => ({
  // https://github.com/tradingview/lightweight-charts/blob/master/docs/area-series.md
  lineColor: "#5472cc",
  topColor: "rgba(49, 69, 131, 0.4)",
  bottomColor: "rgba(42, 64, 103, 0.0)",
  lineWidth: 2,
  priceLineColor: "#231f20",
  downColor: "#fa3c58",
...
export default function ExchangeTVChart(props) {
  ...
  useEffect(() => {
    const minMove = parseFloat(1/(Math.pow(10,chartToken.displayDecimals)));
    ...
    const series = chart.addCandlestickSeries(getSeriesOptions(chartToken.displayDecimals,minMove));
  ...
  useEffect(() => {
    if (currentSeries && priceData && priceData.length) {
      const minMove = parseFloat(1/(Math.pow(10,chartToken.displayDecimals)));
      currentSeries.applyOptions({  priceFormat: {
            type: 'price',
            precision: chartToken.displayDecimals,
            minMove: minMove,
        },})
    ...
    if (currentSeries && savedShouldShowPositionLines) {
        ...
        currentOrders.forEach((order) => {
          ...
          lines.push(
            currentSeries.createPriceLine({
              price: parseFloat(formatAmount(order.triggerPrice, USD_DECIMALS, indexToken.displayDecimals)),
              color,
      ...
      if (positions && positions.length > 0) {
        ...
        positions.forEach((position) => {
          lines.push(
            currentSeries.createPriceLine({
              price: parseFloat(formatAmount(position.averagePrice, USD_DECIMALS, position.indexToken.displayDecimals)),
              color,
          ...
          lines.push(
            currentSeries.createPriceLine({
              price: parseFloat(formatAmount(liquidationPrice, USD_DECIMALS, position.indexToken.displayDecimals)),
              color,
  ...
  const candleStatsHtml = useMemo(() => {
    ...
    const className = cx({
      ...
      [`length-${(String(parseInt(candlestick.close)).length+chartToken.displayDecimals-2)}`]: true,
    });

    const toFixedNumbers = chartToken.displayDecimals;
          ...
          <div>
            <div className="ExchangeChart-title">
              ...
              />
					...
					<div>
						<div className="ExchangeChart-main-price">{chartToken.maxPrice && formatAmount(chartToken.maxPrice, USD_DECIMALS, chartToken.displayDecimals)}</div>
						<div className="ExchangeChart-info-label">${chartToken.minPrice && formatAmount(chartToken.minPrice, USD_DECIMALS, chartToken.displayDecimals)}</div>
					</div>
					<div>
