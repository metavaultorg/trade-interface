...
export default function ConfirmationBox(props) {
  ...
  if (isSwap) {
    ...
    fromTokenUsd = fromTokenInfo ? formatAmount(fromTokenInfo.minPrice, USD_DECIMALS, fromTokenInfo.displayDecimals, true) : 0;
        ...
        if (isMarketOrder && existingPosition.delta.eq(0) && existingPosition.pendingDelta.gt(0)) {
          ...
          return (
            <div className="Confirmation-box-warning">
              ...
              <br />
              Profit price: {existingPosition.isLong ? ">" : "<"} ${formatAmount(profitPrice, USD_DECIMALS, existingPosition.indexToken.displayDecimals, true)}.
  ...
  const renderExecutionFee = useCallback(() => {
    ...
    return (
      <ExchangeInfoRow label="Execution Fee">
        {formatAmount(executionFee, 18, 2)} {getNativeToken(chainId).symbol}
      </ExchangeInfoRow>
    );
      ...
      <>
        <div className="Confirmation-box-info">
          ...
          <ExchangeInfoRow label="Liq. Price">
            {hasExistingPosition && toAmount && toAmount.gt(0) && (
              <div className="inline-block muted">
                ${formatAmount(existingLiquidationPrice, USD_DECIMALS, (toTokenInfo ? toTokenInfo.displayDecimals:2), true)}
                <BsArrowRight className="transition-arrow" />
              </div>
            )}
          ...
          <ExchangeInfoRow label="Fees">${formatAmount(feesUsd, USD_DECIMALS, USD_DISPLAY_DECIMALS, true)}</ExchangeInfoRow>
