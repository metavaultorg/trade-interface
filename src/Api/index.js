...
import { getContract, MVD_TREASURY_ADDRESS, MVX_MULTISIG_ADDRESS, TREASURY_ADDRESS } from "../Addresses";
...
export function useTradesFromGraph(chainId, account) {
  ...
  useEffect(() => {
    ...
    if(queryString){
      setInterval(()=>getMvxGraphClient(chainId).query({ query }).then(setTrades),1000)
    }else{
...
export function useMvxPrice(libraries, active) {
  const polygonLibrary = libraries && libraries.polygon ? libraries.polygon : undefined;
  const { data: mvxPrice, mutate: mutateFromPolygon } = useMvxPriceFromPolygon(polygonLibrary, active);
...
export function useTotalMvxInLiquidity() {
  let poolAddressPolygon = getContract(POLYGON, "UniswapMvxUsdcPool");

  let totalMvx = useRef(bigNumberify(0));

  const { data: mvxInLiquidityOnPolygon, mutate: mutateMvxInLiquidityOnPolygon } = useSWR(
    [`StakeV2:mvxInLiquidity:${POLYGON}`, POLYGON, getContract(POLYGON, "MVX"), "balanceOf", poolAddressPolygon],
    {
      fetcher: fetcher(undefined, Token),
    }
  );
  ...
  const mutate = useCallback(() => {
    mutateMvxInLiquidityOnPolygon();
  }, [mutateMvxInLiquidityOnPolygon]);

  if (mvxInLiquidityOnPolygon) {
    let total = bigNumberify(mvxInLiquidityOnPolygon);
    totalMvx.current = total;
  }
  return {
    polygon: mvxInLiquidityOnPolygon,
...
function useMvxPriceFromPolygon(library, active) {
  const poolAddress = getContract(POLYGON, "UniswapMvxUsdcPool");
