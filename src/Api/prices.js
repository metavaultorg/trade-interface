...
function getCandlePricesFromGraph(tokenSymbol, period) {
  ...
  for (let i = 0; i < CHUNKS_TOTAL; i++) {
    const query = gql(`{
      candlePrices(
        ...
        orderBy: timestamp,
        orderDirection: desc,
        where: {token: "${token.address.toLowerCase()}" period: "${period}", timestamp_lte: ${nowTs} }
      ) {
        timestamp,
        h,
        l,
  ...
  .then((chunks) => {
    ...
    chunks.forEach((chunk) => {
      chunk.data.candlePrices.forEach((item) => {

        const price = {
          time: item.timestamp + timezoneOffset, 
          open: Number(item.o) / 1e8, 
          close: Number(item.c) / 1e8, 
          high: Number(item.h) / 1e8, 
          low: Number(item.l) / 1e8
        }
