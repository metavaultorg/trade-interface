...
function Referrals({ connectWallet, setPendingTxns, pendingTxns }) {
  ...
  function renderAffiliatesTab() {
    ...
    if (referralsData && referralsData.codes && (referralsData.codes?.length > 0 || recentlyAddedCodes.filter(isRecentReferralCodeNotExpired).length > 0)) {
      return (
        <AffiliatesInfo
          account={account}
          active={active}
      ...
      <div className="default-container page-layout Referrals">
        <div className="section-title-block">
          ...
          <div className="section-title-content">
            ...
            Earn commissions and benefit from fee-cashback through our referral program.
              <br />
              For more details, see the{" "}
              <a target="_blank" rel="noopener noreferrer" href="https://docs.metavault.trade/" className="ahreftextcolor">
              referral program.
              </a>
              .
            </div>
    ...
    <div className="referral-body-container">
      <div className="referral-stats">
        ...
        <InfoCard
          ...
          tooltipText="Fee-cashback earned by this account as an affiliate."
            ...
            <div className="referral-table-header">
              <p className="title">
                ...
                <span className="sub-title">
                  {referrerTierInfo && `Tier ${getTierIdDisplay(tierId)} (${tierRebateInfo[tierId]}% fee-commisions)`}
                </span>
              </p>
                    ...
                    <tr key={index}>
                      <td data-label="Referral Code">
                        <div className="table-referral-code">
                          <div
                            onClick={() => {
                              copyToClipboard(`https://metavault.trade?${REFERRAL_CODE_QUERY_PARAMS}=${stat.referralCode}`);
                          ...
                          {codeOwner && !codeOwner?.isTaken && (
                            <div className="info">
                              <Tooltip
                                ...
                                renderContent={() => (
                                  <div>
                                    This code is not yet registered on Polygon
                                    there.
                                  </div>
                                )}
                              />
                          ...
                          {codeOwner && codeOwner?.isTaken && !codeOwner?.isTakenByCurrentUser && (
                            <div className="info">
                              <Tooltip
                                ...
                                renderContent={() => (
                                  <div>
                                    This code has been taken by someone else on Polygon, you will not receive fee-cashback
                      ...
                      <td data-label="Total Fee-commisions">{getUSDValue(referrerRebate)}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      {rebateDistributions?.length > 0 ? (
        <div className="reward-history">
          <Card title="Fee-commisions Distribution History" tooltipText="Fee-commisions are airdropped weekly.">
            <div className="table-wrapper">
              <table className="referral-table">
                <thead>
                  <tr>
        ...
        <EmptyMessage tooltipText="Fee-commisions are airdropped weekly." message="No fee-commisions distribution history yet." />
      )}
    </div>
  );
}
  ...
  return (
    <div className="rebate-container">
      ...
      {discountDistributions.length > 0 ? (
        ...
        <EmptyMessage message="No fee-cashback distribution history yet." tooltipText="Fee-cashbacks are airdropped weekly." />
      )}
    </div>
  );
}
...
function EmptyMessage({ message = "", tooltipText }) {
  return (
    <div className="empty-message mt-5">
      {tooltipText ? (
        <Tooltip className="mt-2" handle={<p>{message}</p>} position="center-bottom" renderContent={() => tooltipText} />
      ) : (
        <p>{message}</p>
      )}
    </div>
