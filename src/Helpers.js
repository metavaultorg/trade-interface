...
export const STABLE_SWAP_FEE_BASIS_POINTS = 15;
...
export const platformTokens = {
  137: {
    ...
    MVX: {
      ...
      imageUrl: "", // TODO: add MVX image url
    },
    MVLP: {
      name: "MVX LP",
      symbol: "MVLP",
      ...
      imageUrl: "", // TODO: add MVLP image url
    },
  },
};
...
export function getExchangeRateDisplay(rate, tokenA, tokenB, opts = {}) {
  ...
  const rateValue = formatAmount(rate, USD_DECIMALS, tokenA.isStable || tokenA.isUsdm ? tokenA.displayDecimals : 4, true);
  if (opts.omitSymbols) {
    return rateValue;
  }
...
export function getNextFromAmount(
  ...
  if (fromTokenAddress === USDM_ADDRESS) {
    ...
    if (ratio && !ratio.isZero()) {
      // apply fees twice usdm -> token1 -> token2
    ...
    // get fromAmount for USDM => stableToken
  ...
  let usdmAmount = fromAmount.mul(fromToken.minPrice).div(PRECISION);
...
export function useAccountOrders(flagOrdersEnabled, overrideAccount) {
  ...
  const {
    ...
    fetcher: async (active, chainId, orderBookAddress, account) => {
      ...
      const fetchIndexesFromServer = () => { return {swap: [], increase: [], decrease: []} }
